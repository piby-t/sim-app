<script lang="ts">
  ...
  const createFreshStatic = () =>
    structuredClone(staticJson.list) as NameValue[];
  const createFreshServer = () =>
    structuredClone(serverJson.list) as SelectArrayText2Props[];
  const createFreshClient = () =>
    structuredClone(clientJson.list) as SelectArrayText2Props[];
  ...
  const apiCatalog = $derived(
    (data.apiCatalog as Record<string, ApiDefinition[]>) || {},
  );
  const keyFiles = $derived(data.keyFiles || []);

  // ✅ 修正: any を排除
  const keyContents = $derived(
    (data.keyContents as Record<string, CredentialRecord>) || {},
  );
  ...
  let selectedServerData = $derived(
    serverPresets.find((p) => p.label === serverState.value),
  );
  let selectedClientData = $derived(
    clientPresets.find((p) => p.label === clientState.value),
  );
  ...
  let allParams = $derived.by(() => {
    ...
    sessionValues.forEach((v) => {
      if (v.name) params[v.name] = v.value;
    });

    // 💡 鍵ファイルの情報を key.xxx として登録
    if (selectedKeyFile && keyContents[selectedKeyFile]) {
    ...
    selectedServerData?.items.forEach((i) => {
      if (i.name) params[i.name] = i.value;
    });
    selectedClientData?.items.forEach((i) => {
      if (i.name) params[i.name] = i.value;
    });
    staticPresets.forEach((i) => {
      if (i.name) params[i.name] = i.value;
    });
  ...
  let resolvedUrl = $derived.by(() => {
    ...
    if (selectedApi.queries && selectedApi.queries.length > 0) {
      ...
      selectedApi.queries.forEach((q) => {
        if (q.name) params.append(q.name, String(processTemplate(q.value)));
      });
  ...
  let resolvedBody = $derived.by(() =>
    !selectedApi || selectedApi.method === "GET" || !selectedApi.body
      ? null
      : processTemplate(selectedApi.body),
  );
  ...
  async function triggerFidoGeneration() {
    ...
    genError = null;
    const challenge =
      sessionValues.find((v) => v.name === "challenge")?.value || "";
    const rpId =
      sessionValues.find((v) => v.name === "rpId")?.value || "localhost";
    const origin =
      sessionValues.find((v) => v.name === "origin")?.value ||
      "http://localhost:3000";
  ...
  $effect(() => {
    if (
      selectedApi &&
      selectedKeyFile &&
      calculatedFlags !== undefined &&
      manualSignCount !== undefined
    ) {
  ...
  <Row class="g-3">
      ...
      <Card class="shadow-sm border-0">
        <CardHeader class="bg-indigo text-white py-2">
          <h6 class="mb-0 fw-bold small">
            <i class="bi bi-shield-lock me-2"></i>FIDO 認証(Login)
            リクエスト設定
          </h6>
        </CardHeader>
        <CardBody>
          <div class="mb-3">
            ...
            <select
              ...
              onchange={(e) => {
                ...
                selectedApi =
                  apiCatalog[group]?.find((a) => a.fileName === fileName) ||
                  null;
              }}
            >
                ...
                <optgroup label={group}>
                  {#each list as api (api.fileName)}
                    <option value="{group}|{api.fileName}"
                      >{api.display || api.fileName}</option
                    >
                  {/each}
                </optgroup>
              {/each}
          ...
          <div class="mb-3 pb-3 border-bottom">
            <KeyFileSelector {keyFiles} bind:selectedKeyFile />
            {#if selectedKeyFile}
              <div class="row g-2 mt-2 bg-light p-2 rounded border mx-0">
                  ...
                  <div class="d-flex flex-wrap gap-3 mt-2">
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fUP"
                        bind:checked={flagUP}
                      /><label class="form-check-label small" for="fUP"
                        >UP</label
                      >
                    </div>
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fUV"
                        bind:checked={flagUV}
                      /><label class="form-check-label small" for="fUV"
                        >UV</label
                      >
                    </div>
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fBE"
                        bind:checked={flagBE}
                      /><label class="form-check-label small" for="fBE"
                        >BE</label
                      >
                    </div>
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fBS"
                        bind:checked={flagBS}
                      /><label class="form-check-label small" for="fBS"
                        >BS</label
                      >
                    </div>
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fAT"
                        bind:checked={flagAT}
                      /><label class="form-check-label small" for="fAT"
                        >AT</label
                      >
                    </div>
                    <div class="form-check form-switch">
                      <input
                        ...
                        id="fED"
                        bind:checked={flagED}
                      /><label class="form-check-label small" for="fED"
                        >ED</label
                      >
                    </div>
                  </div>
                ...
                <div class="col-12 border-top pt-2">
                  <label for="scIn" class="small fw-bold text-muted mb-1"
                    >SignCount (Next Value)</label
                  >
                  <input
                    id="scIn"
                    type="number"
                    class="form-control form-control-sm font-monospace w-50"
                    bind:value={manualSignCount}
                    min="0"
                  />
                </div>
              </div>
          ...
          <div class="mb-3">
            ...
            <SelectArrayText2
              bind:presets={serverPresets}
              bind:selectedLabel={serverState.value}
            />
          </div>
          <div class="mb-3 pb-3 border-bottom">
            <h6 class="text-indigo mb-1 small fw-bold">クライアント設定</h6>
            <SelectArrayText2
              bind:presets={clientPresets}
              bind:selectedLabel={clientState.value}
            /><ClientBasic
              clientData={selectedClientData}
              bind:result={secretBase64}
            />
          </div>
          <div class="mb-1">
            <h6 class="text-indigo mb-1 small fw-bold">
              Static設定 (共通変数)
            </h6>
    ...
    <Col md="6">
      <Card class="shadow-sm border-dark h-100 sticky-top" style="top: 1rem;">
        <CardHeader
          class="bg-dark text-white py-2 d-flex justify-content-between align-items-center"
        >
          <h6 class="mb-0 fw-bold small">送信電文プレビュー</h6>
          {#if selectedApi}<Badge color="warning" class="text-dark"
              >{selectedApi.method}</Badge
            >{/if}
        </CardHeader>
        <CardBody class="bg-light p-3 font-monospace overflow-auto">
          {#if genError}<div class="alert alert-danger small py-2 mb-3">
              {genError}
            </div>{/if}
          {#if selectedApi}
            <div class="mb-3 border-bottom pb-1">
              <div class="fw-bold small text-muted">URL</div>
              <div class="text-break text-primary small">{resolvedUrl}</div>
            </div>
            <div class="mb-3">
              <div class="fw-bold small border-bottom mb-1 text-muted">
                Headers
              </div>
              <Table size="sm" class="bg-white border mb-0 small">
                <tbody
                  >{#each resolvedHeaders as h, i (`${h.name}-${i}`)}<tr
                      ><td class="fw-bold" style="width:35%">{h.name}</td><td
                        class="text-break">{h.value}</td
                      ></tr
                    >{/each}</tbody
                >
              </Table>
            ...
            <div class="mb-3">
              ...
              <pre class="p-2 bg-white border rounded small"><code
                  >{typeof resolvedBody === "string"
                    ? resolvedBody
                    : JSON.stringify(resolvedBody, null, 2)}</code
                ></pre>
            </div>
            <div class="d-grid mt-4">
              <Button
                ...
                onclick={handleExecute}>API EXECUTE</Button
              >
            </div>
          {:else}
