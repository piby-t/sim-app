<script lang="ts">
  ...
  // ✅ 修正: compiler warning (state_referenced_locally) 対策
  // 初期値としてのみ使用し、ページ遷移後の同期は $effect で行います
  ...
  /**
   * ✅ 修正: bind_invalid_expression 回避用のアクセサ配列
   */
  const flagConfigs = $derived([
    { id: 'flagUP', label: 'UP', get val() { return flagUP }, set val(v) { flagUP = v } },
    { id: 'flagUV', label: 'UV', get val() { return flagUV }, set val(v) { flagUV = v } },
    { id: 'flagBE', label: 'BE', get val() { return flagBE }, set val(v) { flagBE = v } },
    { id: 'flagBS', label: 'BS', get val() { return flagBS }, set val(v) { flagBS = v } },
    { id: 'flagAT', label: 'AT', get val() { return flagAT }, set val(v) { flagAT = v } },
    { id: 'flagED', label: 'ED', get val() { return flagED }, set val(v) { flagED = v } }
  ]);
  ...
  // 鍵選択時のデータ復元、およびデータロード時の初期値セット
  $effect(() => {
    // 初回ロード時または data が更新された時に latestKey をセット
