...
import type { KeyMasterConfig, CredentialRecord, FidoProfile } from '$lib/types';
...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        const activeProfile = keyMaster.activeProfile;
        const profile = keyMaster[activeProfile] as FidoProfile;
        if (!profile) throw new Error(`Active profile "${activeProfile}" is missing in config`);

        // 💡 修正ポイント: 外部パス解決 (配布パッケージ直下の data/ を参照)
        const keyPath = path.resolve(process.cwd(), 'data', 'fido', 'key', keyFileName);
        if (!fs.existsSync(keyPath)) {
            throw new Error(`Credential file not found: ${keyPath}`);
        }
        ...
        // 1. Authenticator Data の構築 (37バイト)
        ...
        // 2. 署名対象 (Data to Sign) の構築 (AuthData + ClientDataHash)
        // Base64URL をバイナリに戻してからハッシュ化
        const clientDataBinary = new TextEncoder().encode(atob(clientDataJSON.replace(/-/g, '+').replace(/_/g, '/')));
        const clientDataHash = new Uint8Array(await crypto.subtle.digest("SHA-256", clientDataBinary));
